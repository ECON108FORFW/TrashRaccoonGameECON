...
export function showEnd() {
  ...
  openCard(`<div class="glitter-title" style="font-size:clamp(34px,8vw,54px)">Overall Results</div><p class="end-night-body">Three nights complete. Here’s how the whole run turned out.</p><div class="tags3">${minis}</div><div class="legend"><i style="background:#8a7b5c"></i>What you paid <i style="background:#d85f9a"></i>What it was worth to them</div><p class="tally">Gifts given: <b>${giftsGiven}</b> / 3${missed ? `<br>Missed gifts: <b>${missed}</b>` : ''}<br>Total effort spent: <b>${cost}</b> pebbles<br>Total value to friends: <b>${worth}</b> pebbles<br>Total hearts earned: <b>${hearts}</b> / 9<br>Average hearts per gift: <b>${avgHearts}</b><br>Gift-value efficiency: <b>${efficiency}%</b></p><p class="finale">${netLine}</p><div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap"><button class="btn big" data-go="again">↻ Play again</button><button class="btn big" data-go="economic">Economic lesson →</button></div>`, 40);
}

export function showEconomicLesson() {
  openCard(`<div class="glitter-title" style="font-size:clamp(34px,8vw,54px)">Economic Lesson</div><div class="how-to-body" style="max-width:760px;margin:18px auto 24px;line-height:1.55">This is a high-density problem: a choice that looks simple but depends on many factors at once. The value of anything, whether it's a gift, a job, or a place to live, depends on who it's for, when, where, and what they already have. To make problems easier, we often assume those factors away, but they still affect the outcome, and some of them only the other person knows. That's why a choice made for someone else, like picking a gift, often ends up worth less than it cost.</div><button class="btn big" data-go="end">← Overall results</button>`, 40);
}
