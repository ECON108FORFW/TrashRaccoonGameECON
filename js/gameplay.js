...
import { resetStats, advanceGameTime } from './stats.js';
import { showIntro, showReveal, showEndOfNight } from './overlays.js';
...
export function newGame(){resetGameState();resetStats();setupRound();}
export function setupRound(){
  ...
  Object.assign(S,{fr,factors:new Set(factors),taste:{item:tItem,mult:tMult},tiles:placed,known:new Set(),found:[],digs:fr.digs,sel:null,prevEst:{},lastDelta:{},newChip:null});
  setStatus(`Tap a pile of trash to dig through it. You’ve got <b>${fr.digs} digs</b> before sunrise.`,'');renderUI();showIntro();
}
export function dig(tile){
  const S=gameState.session;if(tile.open||S.digs<=0||gameState.overlayOpen)return;tile.open=true;tile.openedAt=performance.now();S.digs--;advanceGameTime(showEndOfNight);
  ...
  if(S.digs===0)msg+='<br><b>The sky’s getting light. No digs left.</b> Pick a gift from your shelf, or give pebbles.';setStatus(msg,cls);if(S.digs===0)setTimeout(()=>$('shelf').scrollIntoView({behavior:'smooth',block:'center'}),700);renderUI();
}
...
export function give(kind){
  const S=gameState.session,fr=S.fr,X=fr.short||fr.name;let res;
  ...
  else{const id=S.sel,it=ITEMS[id],v=trueValue(id),why=[];S.factors.forEach(f=>{const m=FACTORS[f].eff(it,S.factors);if(Math.abs(m-1)>.01){let knew=S.known.has(f),txt=fill(FACTORS[f].know,fr);if(f==='cold'&&S.factors.has('bare')){txt=`A cold snap is coming AND ${X} has no bedding. Together, that made warm soft things worth way more`;knew=S.known.has('cold')&&S.known.has('bare');why.push(`<li class="${knew?'k':'u'}">${knew?'✔ You knew both':'✘ You didn’t know both'}: ${esc(txt)}.</li>`);return;}why.push(`<li class="${knew?'k':'u'}">${knew?'✔ You knew':'✘ You never found out'}: ${esc(txt)} <b>${m>1?'(made it worth more)':'(made it worth less)'}</b></li>`);}});if(S.taste.item===id)why.push(`<li class="s">🤫 No clue could have told you: ${esc(X)} ${S.taste.mult>1?'just secretly loves things like this':'privately isn’t a fan of things like this'}.</li>`);if(!why.length)why.push(`<li>Nothing special about it to ${esc(X)}. It was just a ${esc(it.n.toLowerCase())}.</li>`);const unknownOther=[...S.factors].filter(f=>!S.known.has(f)&&Math.abs(FACTORS[f].eff(it,S.factors)-1)<.01);res={who:fr.name,short:X,kind,id,e:it.e,n:it.n,cost:it.p,worth:v,hearts:heartsFor(v),line:reactionFor(v/it.p).replace('{item}',it.n.toLowerCase()),why,unknownOther:unknownOther.length};}
  S.results.push(res);showReveal(res);
}
export function nextRound(){gameState.session.round++;setupRound();}
export function noDigsMessage(){setStatus('No digs left — the sun’s coming up. Choose a gift from your shelf, or give pebbles.','');}
