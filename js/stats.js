...
export function renderStats() {
  const tv=$('timeValue'), hv=$('hoursLeftValue');
  if(tv) tv.textContent=formatGameTime(START_HOUR+gameState.elapsedGameHours);
  if(hv){const left=Math.max(0,12-gameState.elapsedGameHours);hv.textContent=`${left} ${left===1?'hour':'hours'} left until 7:00 AM`;}
...
export function resetStats(){ gameState.elapsedGameHours=0; gameState.carryPounds=0; renderStats(); }
export function advanceGameTime(onNightEnd) {
  if(gameState.endNightShown) return;
  gameState.elapsedGameHours += HOURS_PER_DIG;
  renderStats();
  if(gameState.elapsedGameHours>=12){gameState.endNightShown=true;onNightEnd?.();}
}
